...
- **Layer 4:** pass-through phase/round **envelopes** and **expected-next-action metadata** — no phantom `TBoard` on types that do not use it; **`RoundState`** is **not** generic (**`events`** widened to **`Event<Board, EventType>[]`** — the old per-board event correlation was mostly decorative). **`GameState`** is a **discriminated union** (**`StandardGameState | SmallGameState | LargeGameState`**) on a **root** **`boardType`** literal (`'standard' | 'small' | 'large'`) that mirrors **`boardState.boardType`** (enforced in Zod via **`superRefine`**); **`GameStateWithBoard<TBoard extends Board>`** threads **`boardType`** and **`boardState`** together with **`TBoard`**. Persistence ports still take wide **`GameState`**; call sites use **`as GameState`** where the value is typed as **`GameStateWithBoard<BoardForGameType<T>>`** (generic `T` does not assign to the union without that assertion).
...
- **`GameState`** = **`StandardGameState | SmallGameState | LargeGameState`**, discriminated by **root** **`boardType`** (same values as **`boardState.boardType`**). **`GameStateWithBoard<TBoard>`** is **`GameStateBase & { boardType: …; boardState: TBoard }`** so a naked **`TBoard extends Board`** keeps **`boardType`**, **`boardState`**, and **`UnitPlacement<TBoard>`** correlated. **Traits** (setup, victory, rules) are **not** on **`GameState`** branches — they attach to **`Game`** / **`gameType`** (Layer 5 DU; per-variant traits still optional). **`gameStateSchema`** is **`z.discriminatedUnion('boardType', …)`** plus **`superRefine`** for root/nested agreement; per-board **`gameStateSchemaFor*Board`** schemas remain for narrow parses.
...
- **`Game`** = **`StandardGame | MiniGame | TutorialGame`** on **`gameType`**; **`GameOfType<T>`** = **`Extract<Game, { gameType: T }>`**. Each variant repeats the full field list **and JSDoc** (no `extends` for shared fields) so IDE hover shows documentation **without** jumping to a base type; Zod reuses a shared field object for parity only.
