...
describe("getForwardSpace", () => {
  describe("standard board", () => {
    ...
    it("given coordinate missing dash, throws", () => {
      expect(() =>
        getForwardSpace(standardBoard, "invalid" as StandardBoardCoordinate, "north"),
      ).toThrow(new Error("Invalid coordinate: invalid"));
    });

    it("given invalid row letter, throws", () => {
      expect(() =>
        getForwardSpace(standardBoard, "R-12" as StandardBoardCoordinate, "north"),
      ).toThrow(new Error("Invalid row: R"));
    });

    it("given invalid column, throws", () => {
      expect(() =>
        getForwardSpace(standardBoard, "A-19" as StandardBoardCoordinate, "north"),
      ).toThrow(new Error("Invalid column: 19"));
    });

    it("given invalid facing, throws", () => {
      expect(() => getForwardSpace(standardBoard, "E-9", "random" as UnitFacing)).toThrow(
  ...
  describe("small board", () => {
    ...
    it("given coordinate missing dash, throws", () => {
      expect(() => getForwardSpace(smallBoard, "E5" as SmallBoardCoordinate, "north")).toThrow(
        new Error("Invalid coordinate: E5"),
      );
    });

    // Validation tests - rows I, J, K, L don't exist in small board
    it("given row I on small board, throws", () => {
      expect(() => getForwardSpace(smallBoard, "I-5" as SmallBoardCoordinate, "north")).toThrow(
        new Error("Invalid row: I"),
      );
    });
    it("given row L on small board, throws", () => {
      expect(() => getForwardSpace(smallBoard, "L-5" as SmallBoardCoordinate, "north")).toThrow(
        new Error("Invalid row: L"),
      );
    });

    // Validation tests - columns 13-18 don't exist in small board
    it("given column 13 on small board, throws", () => {
      expect(() => getForwardSpace(smallBoard, "A-13" as SmallBoardCoordinate, "north")).toThrow(
        new Error("Invalid column: 13"),
      );
    });
    it("given column 18 on small board, throws", () => {
      expect(() => getForwardSpace(smallBoard, "A-18" as SmallBoardCoordinate, "north")).toThrow(
        new Error("Invalid column: 18"),
      );
    });

    it("given invalid facing, throws", () => {
      expect(() => getForwardSpace(smallBoard, "E-9", "random" as UnitFacing)).toThrow(
        new Error("Invalid facing: random"),
      );
    });
  });
