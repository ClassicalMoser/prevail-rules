...
describe("getDiagonallyAdjacentSpaces", () => {
  ...
  it("given edge coordinate, returns fewer diagonals", () => {
    expect(getDiagonallyAdjacentSpaces(standardBoard, "A-5")).toEqual(new Set(["B-4", "B-6"]));
  });

  it("given corner coordinate, returns one diagonal", () => {
    expect(getDiagonallyAdjacentSpaces(standardBoard, "L-18")).toEqual(new Set(["K-17"]));
  });
