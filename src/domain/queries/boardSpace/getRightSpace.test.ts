...
describe("getRightSpace", () => {
  ...
  it("given invalid row letter, throws", () => {
    expect(() => getRightSpace(standardBoard, "R-12" as StandardBoardCoordinate, "north")).toThrow(
      new Error("Invalid row: R"),
    );
  });

  it("given invalid column, throws", () => {
    expect(() => getRightSpace(standardBoard, "A-19" as StandardBoardCoordinate, "north")).toThrow(
      new Error("Invalid column: 19"),
    );
  });

  it("given invalid facing, throws", () => {
    expect(() => getRightSpace(standardBoard, "E-9", "random" as UnitFacing)).toThrow(
