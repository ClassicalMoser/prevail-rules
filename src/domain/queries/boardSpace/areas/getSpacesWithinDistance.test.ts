...
describe(getSpacesWithinDistance, () => {
  ...
  it('given distance 2 from interior, includes two-step ring', () => {
    ...
    expect(result.has('E-5')).toBeTruthy();
    expect(result.has('D-5')).toBeTruthy();
    expect(result.has('C-5')).toBeTruthy();
    expect(result.has('D-4')).toBeTruthy();
    expect(result.has('C-3')).toBeTruthy();
    expect(result.has('C-4')).toBeTruthy();
  ...
  it('given distance 1 from corner, returns only in-bounds neighbors', () => {
    ...
    expect(result.has('A-2')).toBeTruthy();
    expect(result.has('B-1')).toBeTruthy();
    expect(result.has('B-2')).toBeTruthy();
    expect(result.size).toBe(4);
  });

  it('given distance 1 from edge, returns six neighbors', () => {
    const result = getSpacesWithinDistance(standardBoard, 'A-5', 1);
    expect(result.has('A-5')).toBeTruthy();
    expect(result.has('A-4')).toBeTruthy();
    expect(result.has('A-6')).toBeTruthy();
    expect(result.has('B-4')).toBeTruthy();
    expect(result.has('B-5')).toBeTruthy();
    expect(result.has('B-6')).toBeTruthy();
  ...
  it('given distance 2, excludes tiles beyond 2 steps', () => {
    const result = getSpacesWithinDistance(standardBoard, 'E-5', 2);
    expect(result.has('B-5')).toBeFalsy();
    expect(result.has('C-5')).toBeTruthy();
  });
