...
describe("getLeftSpace", () => {
  ...
  it("given invalid row letter, throws", () => {
    expect(() => getLeftSpace(standardBoard, "R-12" as StandardBoardCoordinate, "north")).toThrow(
      new Error("Invalid row: R"),
    );
  });

  it("given invalid column, throws", () => {
    expect(() => getLeftSpace(standardBoard, "A-19" as StandardBoardCoordinate, "north")).toThrow(
      new Error("Invalid column: 19"),
    );
  });

  it("given invalid facing, throws", () => {
    expect(() => getLeftSpace(standardBoard, "E-9", "random" as UnitFacing)).toThrow(
