import { createBoardWithUnits, createEmptyGameState, createTestUnit } from "@testing";
...
describe("getPlayerUnitsWithPlacementOnBoard", () => {
  it("given a player, returns all units with placement", () => {
    ...
    expect(whiteUnitsArray.some((u) => u.unit === unit1 && u.placement.coordinate === "E-5")).toBe(
      true,
    );
    expect(whiteUnitsArray.some((u) => u.unit === unit2 && u.placement.coordinate === "F-5")).toBe(
      true,
    );
  });
