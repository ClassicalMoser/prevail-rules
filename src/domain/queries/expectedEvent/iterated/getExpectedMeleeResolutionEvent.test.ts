...
describe("getExpectedMeleeResolutionEvent", () => {
  ...
  it("given delegate to the first player attack apply state when it is incomplete", () => {
    ...
    expect(getExpectedMeleeResolutionEvent(gameState, meleeState)).toBe(expectedEvent);
  ...
  it("given when the melee resolution is already complete, throws", () => {
    ...
    expect(() => getExpectedMeleeResolutionEvent(gameState, meleeState)).toThrow(
      "Melee resolution state is already complete",
    );
  });
});
