...
describe("getLegalUnitMoves", () => {
  ...
  describe("basic functionality", () => {
    it("given context, returns legal moves including starting position", () => {
      ...
      const unit = getPlayerUnitWithPosition(gameState.boardState, "D-7", "black");
      if (!unit) {
  ...
  describe("error cases", () => {
    ...
    it("given an error when unit is not at the specified placement coordinate, throws", () => {
      ...
      const unit = getPlayerUnitWithPosition(gameState.boardState, "E-7", "black");
      if (!unit) {
        throw new Error("Unit not found");
      }
      ...
      expect(() => getLegalUnitMoves(unitWithWrongCoordinate, gameState)).toThrow(
        "No movable unit at starting position",
      );
    });
