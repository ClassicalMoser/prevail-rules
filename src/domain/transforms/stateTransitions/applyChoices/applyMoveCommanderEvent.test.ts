...
describe("applyMoveCommanderEvent", () => {
  ...
  describe("board placement", () => {
    it("given moveFirstCommander and black E-5 to E-7, black leaves E-5 and white still on E-6", () => {
      ...
      expect(newState.boardState.board["E-5"]?.commanders.has("black")).toBe(false);
      // Commander should be at new location
      expect(newState.boardState.board["E-7"]?.commanders.has("black")).toBe(true);
      // White commander should be unchanged
      expect(newState.boardState.board["E-6"]?.commanders.has("white")).toBe(true);
    });
    ...
    it("given moveSecondCommander and white E-6 to E-8, white leaves E-6 and black still on E-5", () => {
      ...
      // Commander should be removed from original location
      expect(newState.boardState.board["E-6"]?.commanders.has("white")).toBe(false);
      // Commander should be at new location
      expect(newState.boardState.board["E-8"]?.commanders.has("white")).toBe(true);
      // Black commander should be unchanged
      expect(newState.boardState.board["E-5"]?.commanders.has("black")).toBe(true);
    });
  });
  ...
  describe("step progression", () => {
    it("given black completes first commander move, phase step is moveSecondCommander", () => {
      ...
      expect(newState.currentRoundState.currentPhaseState?.step).toBe("moveSecondCommander");
    });
