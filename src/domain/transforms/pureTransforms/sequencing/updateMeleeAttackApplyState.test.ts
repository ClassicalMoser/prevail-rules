...
describe("updateMeleeAttackApplyState", () => {
  ...
  it("given update white attack apply state", () => {
    ...
    expect(phase.currentMeleeResolutionState?.blackAttackApplyState?.completed).toBe(false);
  });
  ...
  it("given update black attack apply state", () => {
    ...
    expect(phase.currentMeleeResolutionState?.whiteAttackApplyState?.completed).toBe(false);
  });
  ...
  it("given not mutate the original state", () => {
    ...
    expect(
      state.currentRoundState.currentPhaseState?.phase === "resolveMelee" &&
        state.currentRoundState.currentPhaseState.currentMeleeResolutionState?.whiteAttackApplyState
          ?.completed,
