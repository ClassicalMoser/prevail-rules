...
describe("updateRoutState", () => {
  ...
  function createStateWithMeleeRout(routingPlayer: "white" | "black") {
    ...
    const otherUnit = createTestUnit(routingPlayer === "white" ? "black" : "white", { attack: 2 });
  ...
  it("given update rout state in ranged attack resolution", () => {
    ...
    expect(phase.currentCommandResolutionState.attackApplyState?.routState?.numberToDiscard).toBe(
      2,
    );
  });
  ...
  it("given update rout state in melee resolution for white", () => {
    ...
    expect(
      phase.currentMeleeResolutionState?.whiteAttackApplyState?.routState?.numberToDiscard,
    ).toBe(1);
  });
  ...
  it("given update rout state in melee resolution for black", () => {
    ...
    if (phase?.phase !== "resolveMelee") throw new Error("phase");
    expect(
      phase.currentMeleeResolutionState?.blackAttackApplyState?.routState?.numberToDiscard,
    ).toBe(2);
  });
  ...
  it("given update rout state in cleanup phase (routs from lost support)", () => {
    ...
    expect(phase.firstPlayerRallyResolutionState?.routState?.numberToDiscard).toBe(2);
  });
  ...
  it("given when no rout state in attack apply, throws", () => {
    ...
    expect(() => updateRoutState(stateInPhase, createRoutState("white", unit))).toThrow(
  ...
  it("given when melee white attack apply has no rout state, throws", () => {
    ...
    expect(() => updateRoutState(stateInPhase, createRoutState("white", whiteUnit))).toThrow(
      "No rout state found in attack apply state",
    );
  });
  ...
  it("given when melee black attack apply has no rout state, throws", () => {
    ...
    expect(() => updateRoutState(stateInPhase, createRoutState("black", blackUnit))).toThrow(
      "No rout state found in attack apply state",
    );
  });
  ...
  it("given when cleanup rally resolution has no rout state, throws", () => {
    ...
    expect(() => updateRoutState(stateInCleanup, createRoutState("white", unit))).toThrow(
  ...
  it("given when not in issueCommands, resolveMelee, or cleanup phase, throws", () => {
    ...
    expect(() => updateRoutState(stateInPlayCards, createRoutState("white", unit))).toThrow(
      "Rout state update not expected in phase: playCards",
    );
  });
});
