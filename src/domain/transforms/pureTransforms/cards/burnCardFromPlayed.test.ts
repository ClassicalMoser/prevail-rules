...
describe("burnCardFromPlayed", () => {
  it("given move card from played to burnt", () => {
    ...
    const newCardState = burnCardFromPlayed(cardState, "black", tempCommandCards[0]);
  ...
  it("given if card is not in played pile, throws", () => {
    ...
    expect(() => burnCardFromPlayed(cardState, "black", tempCommandCards[1])).toThrow(
  ...
  it("given append to existing burnt cards", () => {
    ...
    expect(newCardState.black.burnt).toEqual([tempCommandCards[2], tempCommandCards[0]]);
  });
});
