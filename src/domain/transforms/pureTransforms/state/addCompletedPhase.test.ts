...
describe("addCompletedPhase", () => {
  it("given add phase to completed phases set", () => {
    ...
    expect(newState.currentRoundState.completedPhases.has(phaseState)).toBe(true);
  ...
  it("given preserve existing completed phases", () => {
    ...
    expect(stateWithBoth.currentRoundState.completedPhases.has(firstPhase)).toBe(true);
    expect(stateWithBoth.currentRoundState.completedPhases.has(secondPhase)).toBe(true);
  });
});
