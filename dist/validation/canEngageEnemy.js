 ...
 */
export function canEngageEnemy(unit, board, destinationCoordinate, adjacentFacing, adjacentCoordinate, remainingFlexibility, moveStartCoordinate) {
    // Check if the destination has an enemy unit
    let destinationSpace;
    try {
        destinationSpace = getBoardSpace(board, destinationCoordinate);
        if (destinationSpace.unitPresence.presenceType !== "single" ||
