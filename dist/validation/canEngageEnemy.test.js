...
describe("canEngageEnemy", () => {
    ...
    describe("invalid inputs", () => {
        it("should return false for a non-existent destination coordinate", () => {
            ...
            const adjacentCoordinate = "E-4";
            expect(canEngageEnemy(unit, standardBoard, invalidCoordinate, "north", adjacentCoordinate, 1, adjacentCoordinate)).toBe(false);
        });
        it("should return false for a non-existent adjacent coordinate", () => {
            const unit = createUnitInstance("black");
            const board = createBoardWithEnemyUnit(enemyCoordinate, "white");
            const invalidCoordinate = "Z-99";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "north", invalidCoordinate, 1, invalidCoordinate)).toBe(false);
        });
    });
    describe("not an enemy space", () => {
        it("should return false for an empty space", () => {
            const unit = createUnitInstance("black");
            const adjacentCoordinate = "E-4";
            expect(canEngageEnemy(unit, standardBoard, enemyCoordinate, "north", adjacentCoordinate, 1, adjacentCoordinate)).toBe(false);
        });
        it("should return false for a friendly unit", () => {
            const unit = createUnitInstance("black");
            const board = createBoardWithEnemyUnit(enemyCoordinate, "black");
            const adjacentCoordinate = "E-4";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "south", adjacentCoordinate, 1, adjacentCoordinate)).toBe(false);
        });
    });
    describe("coming from flank", () => {
        it("should return true when coming from a flank space", () => {
            ...
            const flankCoordinate = "E-4";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "east", flankCoordinate, 0, flankCoordinate)).toBe(true);
        });
        it("should return true when coming from a flank space with any facing", () => {
            const unit = createUnitInstance("black");
            const board = createBoardWithEnemyUnit(enemyCoordinate, "white", "north");
            const flankCoordinate = "E-6";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "southWest", flankCoordinate, 0, flankCoordinate)).toBe(true);
        });
    });
    describe("coming from back", () => {
        it("should return true when coming from back and started behind enemy", () => {
            ...
            const backCoordinate = "F-5";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "north", backCoordinate, 0, backCoordinate)).toBe(true);
        });
        it("should return false when coming from back but didn't start behind enemy", () => {
            const unit = createUnitInstance("black");
            const board = createBoardWithEnemyUnit(enemyCoordinate, "white", "north");
            // F-6 is a back space
            const backCoordinate = "F-6";
            // Started at C-8, which is not behind the enemy
            const startCoordinate = "C-8";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "southEast", backCoordinate, 0, startCoordinate)).toBe(false);
        });
        it("should return true when coming from back diagonal and started behind enemy", () => {
            ...
            const startCoordinate = "G-7";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "northWest", backCoordinate, 0, startCoordinate)).toBe(true);
        });
    });
    describe("coming from front", () => {
        it("should return true when already facing opposite to enemy", () => {
            ...
            const frontCoordinate = "D-5";
            // Facing south (opposite of north)
            expect(canEngageEnemy(unit, board, enemyCoordinate, "south", frontCoordinate, 0, frontCoordinate)).toBe(true);
        });
        it("should return true when need to rotate and have flexibility", () => {
            const unit = createUnitInstance("black");
            const board = createBoardWithEnemyUnit(enemyCoordinate, "white", "north");
            const frontCoordinate = "D-4";
            // Facing southEast, need to rotate to south, have flexibility
            expect(canEngageEnemy(unit, board, enemyCoordinate, "southEast", frontCoordinate, 1, frontCoordinate)).toBe(true);
        });
        it("should return false when need to rotate but no flexibility", () => {
            ...
            expect(canEngageEnemy(unit, board, enemyCoordinate, "southEast", frontCoordinate, 0, frontCoordinate)).toBe(false);
        });
        it("should handle diagonal front spaces", () => {
            const unit = createUnitInstance("black");
            const board = createBoardWithEnemyUnit(enemyCoordinate, "white", "north");
            // F-5 is a front space
            const frontDiagonalCoordinate = "D-4";
            // Facing south (opposite of north)
            expect(canEngageEnemy(unit, board, enemyCoordinate, "southEast", frontDiagonalCoordinate, 2, frontDiagonalCoordinate)).toBe(true);
        });
        it("should handle different enemy facings", () => {
            ...
            const frontCoordinate = "E-6";
            // Facing west (opposite of east)
            expect(canEngageEnemy(unit, board, enemyCoordinate, "west", frontCoordinate, 0, frontCoordinate)).toBe(true);
        });
    });
    describe("not adjacent to enemy", () => {
        it("should return false when not adjacent to enemy", () => {
            ...
            const nonAdjacentCoordinate = "G-5";
            expect(canEngageEnemy(unit, board, enemyCoordinate, "north", nonAdjacentCoordinate, 1, nonAdjacentCoordinate)).toBe(false);
        });
    });
    describe("edge cases", () => {
        ...
        it("should handle multiple flexibility values", () => {
            ...
            expect(canEngageEnemy(unit, board, enemyCoordinate, "southEast", frontCoordinate, 2, frontCoordinate)).toBe(true);
        });
    });
});
