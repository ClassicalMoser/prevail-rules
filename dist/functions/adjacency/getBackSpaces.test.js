...
describe("getBackSpaces", () => {
    ...
    it("should return the back spaces for diagonal facing directions", () => {
        ...
        expect(getBackSpaces(standardBoard, "E-5", "northEast")).toEqual(new Set(["F-5", "E-4", "F-4"]));
        // For southEast facing, back spaces are the front spaces when facing northWest
        expect(getBackSpaces(standardBoard, "E-5", "southEast")).toEqual(new Set(["E-4", "D-5", "D-4"]));
        // For southWest facing, back spaces are the front spaces when facing northEast
        expect(getBackSpaces(standardBoard, "E-5", "southWest")).toEqual(new Set(["D-5", "E-6", "D-6"]));
        // For northWest facing, back spaces are the front spaces when facing southEast
        expect(getBackSpaces(standardBoard, "E-5", "northWest")).toEqual(new Set(["E-6", "F-5", "F-6"]));
    });
    ...
    it("should throw an error if the row is invalid", () => {
        expect(() => getBackSpaces(standardBoard, "R-12", "north")).toThrow(new Error("Invalid row: R"));
    });
    it("should throw an error if the column is invalid", () => {
        expect(() => getBackSpaces(standardBoard, "A-19", "north")).toThrow(new Error("Invalid column: 19"));
    });
    it("should throw an error if the facing is invalid", () => {
        expect(() => getBackSpaces(standardBoard, "E-9", "random")).toThrow(new Error("Invalid facing: random"));
    });
});
