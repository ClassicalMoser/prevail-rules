...
describe("getBackSpaces", () => {
  ...
  it("should throw an error if the row is invalid", () => {
    expect(() => getBackSpaces(standardBoard, "R-12", "north")).toThrow(
      new Error("Invalid row: R"),
    );
  });
  it("should throw an error if the column is invalid", () => {
    expect(() => getBackSpaces(standardBoard, "A-19", "north")).toThrow(
      new Error("Invalid column: 19"),
    );
  });
  it("should throw an error if the facing is invalid", () => {
    expect(() => getBackSpaces(standardBoard, "E-9", "random")).toThrow(
