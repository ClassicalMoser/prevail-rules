 ...
 */
export function createBoardWithUnits(units) {
 ...
 */
export function createBoardWithSingleUnit(coord, playerSide, options) {
    const board = createEmptyStandardBoard();
    ...
    if (options?.unitType) {
        ...
        if (!unitType) {
            throw new Error(`No unit found with flexibility value ${options.flexibility}.`);
        }
 ...
 */
export function createBoardWithEngagedUnits(primaryUnit, secondaryUnit, coord = "E-5", primaryFacing = "north") {
    const board = createEmptyStandardBoard();
    board.board[coord] = {
        ...board.board[coord],
        unitPresence: {
 ...
 */
export function createBoardWithCommander(playerSide, coordinate, board) {
