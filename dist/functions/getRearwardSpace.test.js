...
describe("getRearwardSpace", () => {
    ...
    it("should throw an error if the row is invalid", () => {
        expect(() => getRearwardSpace("R12", "north")).toThrow(new Error("Invalid row: R"));
    });
    it("should throw an error if the column is invalid", () => {
        expect(() => getRearwardSpace("A19", "north")).toThrow(new Error("Invalid column: 19"));
    });
    it("should throw an error if the facing is invalid", () => {
        expect(() => getRearwardSpace("E9", "random")).toThrow(new Error("Invalid facing: random"));
    });
});
