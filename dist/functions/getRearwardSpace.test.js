...
describe("getRearwardSpace", () => {
    ...
    it("should throw an error if the row is invalid", () => {
        expect(() => getRearwardSpace(standardBoard, "R-12", "north")).toThrow(new Error("Invalid row: R"));
    });
    it("should throw an error if the column is invalid", () => {
        expect(() => getRearwardSpace(standardBoard, "A-19", "north")).toThrow(new Error("Invalid column: 19"));
    });
    it("should throw an error if the facing is invalid", () => {
        expect(() => getRearwardSpace(standardBoard, "E-9", "random")).toThrow(new Error("Invalid facing: random"));
    });
});
