...
describe("getAdjacentFacings", () => {
    it("should return the adjacent facings for orthogonal facing directions", () => {
        expect(getAdjacentFacings("north")).toEqual(new Set(["northWest", "northEast"]));
        expect(getAdjacentFacings("east")).toEqual(new Set(["northEast", "southEast"]));
        expect(getAdjacentFacings("south")).toEqual(new Set(["southEast", "southWest"]));
        expect(getAdjacentFacings("west")).toEqual(new Set(["southWest", "northWest"]));
    });
    ...
    it("should throw an error if the facing is invalid", () => {
        expect(() => getAdjacentFacings("invalid")).toThrow(new Error("Invalid facing: invalid"));
    });
});
