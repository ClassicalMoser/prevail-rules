...
export function getLegalUnitMoves(unit, board, startingPosition) {
    ...
    const getStateKey = (coordinate, facing, remainingSpeed, remainingFlexibility) => {
        return `${coordinate}|${facing}|${remainingSpeed}|${remainingFlexibility}`;
    };
    // Recursive function to explore all possible move sequences
    const exploreMoves = (currentCoordinate, currentFacing, remainingSpeed, remainingFlexibility, previousCoordinate = undefined) => {
        // Create state key for this position
        const stateKey = getStateKey(currentCoordinate, currentFacing, remainingSpeed, remainingFlexibility);
        ...
        const canEndHere = isStartingPosition || canMoveInto(unit, board, currentCoordinate);
        if (canEndHere) {
            ...
            const isEnemySpace = currentSpace.unitPresence.presenceType === "single" &&
            ...
            if (isEnemySpace && previousCoordinate !== undefined) {
                ...
                if (canEngageEnemy(unit, board, currentCoordinate, currentFacing, previousCoordinate, remainingFlexibility, startingPosition.coordinate)) {
                    legalMoves.add({
        ...
        if (remainingSpeed > 0) {
            const forwardCoordinate = getForwardSpace(board, currentCoordinate, currentFacing);
            if (forwardCoordinate !== undefined) {
                ...
                if (isDiagonalFacing(currentFacing)) {
                    ...
                    const validPassThroughSpaces = orthogonalPassThroughSpaces.filter((space) => canMoveThrough(unit, board, space));
                ...
                if (canContinue) {
                    ...
                    exploreMoves(forwardCoordinate, currentFacing, remainingSpeed - 1, remainingFlexibility, currentCoordinate);
                }
                else if (canEnd) {
                    ...
                    exploreMoves(forwardCoordinate, currentFacing, 0, remainingFlexibility, currentCoordinate);
                }
            }
        }
        ...
        if (remainingFlexibility > 0) {
            for (const newFacing of unitFacings) {
                ...
                if (newFacing !== currentFacing) {
                    exploreMoves(currentCoordinate, newFacing, remainingSpeed, remainingFlexibility - 1, previousCoordinate);
                }
            }
        }
    };
    // Start exploring from the starting position
    exploreMoves(startingPosition.coordinate, startingPosition.facing, unit.unitType.speed, unit.unitType.flexibility);
    return legalMoves;
}
