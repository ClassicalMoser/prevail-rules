...
describe("getFrontSpaces", () => {
    ...
    it("should return the front spaces for diagonal facing directions", () => {
        ...
        expect(getFrontSpaces(standardBoard, "E-5", "northEast")).toEqual(new Set(["D-5", "E-6", "D-6"]));
        // For southEast facing, front spaces are: east, south, southEast (in that order)
        expect(getFrontSpaces(standardBoard, "E-5", "southEast")).toEqual(new Set(["E-6", "F-5", "F-6"]));
        // For southWest facing, front spaces are: south, west, southWest (in that order)
        expect(getFrontSpaces(standardBoard, "E-5", "southWest")).toEqual(new Set(["F-5", "E-4", "F-4"]));
        // For northWest facing, front spaces are: west, north, northWest (in that order)
        expect(getFrontSpaces(standardBoard, "E-5", "northWest")).toEqual(new Set(["E-4", "D-5", "D-4"]));
    });
    ...
    it("should throw an error if the row is invalid", () => {
        expect(() => getFrontSpaces(standardBoard, "R-12", "north")).toThrow(new Error("Invalid row: R"));
    });
    it("should throw an error if the column is invalid", () => {
        expect(() => getFrontSpaces(standardBoard, "A-19", "north")).toThrow(new Error("Invalid column: 19"));
    });
    it("should throw an error if the facing is invalid", () => {
        expect(() => getFrontSpaces(standardBoard, "E-9", "random")).toThrow(new Error("Invalid facing: random"));
    });
});
